...
def draw_bsd_indicators(frame: Any, bsd_warnings: List[Any]) -> Any:
    ...
    render = frame.copy()
    h, w = render.shape[:2]
    for warn in bsd_warnings:
        side = getattr(warn, "side", "left")
        level = getattr(warn, "level", "WARNING").upper()
        color = (0, 0, 255) if level == "CRITICAL" else (0, 165, 255)
        # Draw indicator triangle on the appropriate side mirror area
        if side == "left":
            cx, cy = 50, h // 2
        else:
            cx, cy = w - 50, h // 2
        ...
        tri_size = 25
        if side == "left":
            pts = np.array([[cx, cy - tri_size], [cx, cy + tri_size], [cx - tri_size, cy]])
        else:
            pts = np.array([[cx, cy - tri_size], [cx, cy + tri_size], [cx + tri_size, cy]])
        cv2.fillPoly(render, [pts], color)
        # Label
        ttc_txt = f"TTC={getattr(warn, 'ttc', 0):.1f}s" if getattr(warn, "ttc", None) else ""
        label = f"BSD {side.upper()} {ttc_txt}"
