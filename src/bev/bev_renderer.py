...
class BEVRenderer:
    ...
    def _draw_objects(self, canvas: np.ndarray, world: Any, fcw: dict) -> None:
        ...
        for obj in objs:
            ...
            # Prediction points from temporal predictor (Phase 2.3)
            predictions = getattr(world, "predictions", {})
            tid = getattr(obj, "track_id", None)
            if tid is not None and tid in predictions:
                for pred_pt in predictions[tid]:
                    px_pred = getattr(pred_pt, "x_m", None)
                    py_pred = getattr(pred_pt, "y_m", None)
                    if px_pred is not None and py_pred is not None:
                        pp_x, pp_y = self.world_to_bev(px_pred, py_pred)
                        cv2.circle(canvas, (pp_x, pp_y), 4, color, 1)
            else:
                # Fallback: prediction point at ~1s ahead
